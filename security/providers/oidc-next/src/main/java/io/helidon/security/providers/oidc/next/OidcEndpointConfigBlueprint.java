...
interface OidcEndpointConfigBlueprint {
    ...
    /**
     * Whether OpenID Connect endpoint URIs and OIDC outbound bearer-token targets must use transport-layer security.
     * <p>
     * WARNING: Disabling this option permits using non-TLS endpoint URIs and permits Token Propagation or Client
     * Credentials Grant to attach Bearer tokens to non-HTTPS outbound target URIs. This can expose access tokens,
     * client credentials, and token signature verification keys, and is not compliant with OpenID Connect and OAuth
     * endpoint TLS requirements. It should only be used for isolated tests or equivalent non-production environments
     * where the endpoint and key source are controlled.
     * <p>
     ...
     *
     * @return whether OpenID Connect endpoint URIs and OIDC outbound bearer-token targets must use transport-layer security
     */
    @Option.Configured
