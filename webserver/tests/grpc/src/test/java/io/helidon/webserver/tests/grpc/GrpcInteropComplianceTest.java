...
class GrpcInteropComplianceTest extends BaseServiceTest {
    ...
    private static final Metadata.Key<String> INITIAL_METADATA_KEY =
            Metadata.Key.of("x-grpc-test-echo-initial", Metadata.ASCII_STRING_MARSHALLER);
    private static final Metadata.Key<byte[]> TRAILING_METADATA_KEY =
    ...
    static void routing(Router.RouterBuilder<?> router) {
        router.addRouting(GrpcRouting.builder()
                                  .service(ServerInterceptors.intercept(new ComplianceStringService(),
                                                                        new MetadataEchoInterceptor())));
    }
    ...
    void testBidirectionalCustomMetadataRoundTrip() throws InterruptedException {
        ...
        CallResult<Strings.StringMessage> result = streamingCall(StringServiceGrpc.getEchoMethod(),
                                                                 metadata,
                                                                 List.of(message("hello"), message("world")));
    ...
    void testUnaryStatusCodeAndMessage() {
        ...
        StatusRuntimeException exception = assertThrows(StatusRuntimeException.class,
                                                        () -> service.lower(message("bad request")));
    ...
    void testSpecialStatusMessageRoundTrip() {
        ...
        StatusRuntimeException exception = assertThrows(StatusRuntimeException.class,
                                                        () -> service.lower(message(statusMessage)));
    ...
    void testBidirectionalStatusCodeAndMessage() throws InterruptedException {
        ...
        CallResult<Strings.StringMessage> result = streamingCall(StringServiceGrpc.getEchoMethod(),
                                                                 ...
                                                                 List.of(message("hello"),
                                                                         message(ERROR_PREFIX + statusMessage)));
    ...
    void testPingPongStreamingPreservesMessageOrder() throws InterruptedException {
        ...
        call.request(1);
        call.sendMessage(message("three"));
    ...
    void testCompressedUnaryRoundTrip() throws InterruptedException {
        ...
        try {
            StringServiceGrpc.StringServiceBlockingStub gzipStub = StringServiceGrpc.newBlockingStub(gzipChannel)
    ...
    private static final class ComplianceStringService extends StringServiceGrpc.StringServiceImplBase {
        ...
        public void lower(Strings.StringMessage request, StreamObserver<Strings.StringMessage> observer) {
            observer.onError(Status.INVALID_ARGUMENT.withDescription(request.getText()).asRuntimeException());
        }
        ...
        public StreamObserver<Strings.StringMessage> join(StreamObserver<Strings.StringMessage> observer) {
            return new StreamObserver<>() {
                ...
                public void onCompleted() {
                    observer.onNext(message(builder.toString()));
                    observer.onCompleted();
                }
            };
        }
    ...
    private static final class MetadataEchoInterceptor implements ServerInterceptor {
        @Override
        public <ReqT, RespT> ServerCall.Listener<ReqT> interceptCall(ServerCall<ReqT, RespT> call,
                                                                     Metadata headers,
                                                                     ServerCallHandler<ReqT, RespT> next) {
            LAST_REQUEST_ENCODING.set(headers.get(GRPC_ENCODING));
