    ...
    public void decorate(SchemaArray.BuilderBase<?, ?> target) {
        ...
        if (minContains.isPresent() && maxContains.isPresent()) {
            if (minContains.get() > maxContains.get()) {
                throw new JsonSchemaException("Minimum contains value cannot be greater than the maximum value");
            }
        }
        if (minContains.isPresent() && minContains.get() < 0) {
            throw new JsonSchemaException("Minimum contains cannot be lower than 0");
        }
        if (maxContains.isPresent() && maxContains.get() < 0) {
            throw new JsonSchemaException("Maximum contains cannot be lower than 0");
        }
