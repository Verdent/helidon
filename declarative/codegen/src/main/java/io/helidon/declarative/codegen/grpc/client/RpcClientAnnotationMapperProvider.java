...
/**
 * Annotation mapper that makes each declarative gRPC client API a service contract.
 */
public class RpcClientAnnotationMapperProvider implements AnnotationMapperProvider {
    /**
     * Public constructor required by {@link java.util.ServiceLoader}.
     */
    public RpcClientAnnotationMapperProvider() {
    }

    @Override
    public AnnotationMapper create(CodegenOptions options) {
        return new RpcClientAnnotationMapper();
    }

    @Override
    public Set<TypeName> supportedAnnotations() {
        return Set.of(RpcClientTypes.ANNOTATION_ENDPOINT);
    }

    private static class RpcClientAnnotationMapper implements AnnotationMapper {
        private static final Annotation CONTRACT_ANNOTATION = Annotation.create(SERVICE_ANNOTATION_CONTRACT);

        @Override
        public boolean supportsAnnotation(Annotation annotation) {
            return annotation.typeName().equals(RpcClientTypes.ANNOTATION_ENDPOINT);
        }
