...
public interface JsonBinding extends RuntimeType.Api<JsonBindingConfig> {
     ...
     * @return the JSON bytes encoded using UTF-8
     */
    default <T> byte[] serializeToBytes(T obj, Class<? super T> type) {
        return serialize(obj, type).getBytes(StandardCharsets.UTF_8);
    }
     ...
     * @return the JSON bytes encoded using UTF-8
     */
    default <T> byte[] serializeToBytes(T obj, GenericType<? super T> type) {
        return serialize(obj, type).getBytes(StandardCharsets.UTF_8);
    }
