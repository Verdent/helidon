...
@Service.Singleton
class DefaultGrpcClient implements Supplier<GrpcClient> {
    private static final String URI_EXPRESSION = "http://localhost:${test.server.port}";

    private final Config config;
    private volatile GrpcClient client;

    @Service.Inject
    DefaultGrpcClient(Config config) {
        this.config = config;
    }
