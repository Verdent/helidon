...
class RefreshTokenIT extends CommonLoginBase {
    ...
    public void testRefreshToken(WebTarget webTarget) {
        List<String> initialCookies = obtainCookies(webTarget);
        //        String formUri;
//
...
//        }
        ...
        Invocation.Builder requestBuilder = client2.target(webTarget.getUri())
                .path("/test")
                .request();

        for (String cookie : initialCookies) {
            if (cookie.startsWith(OidcConfig.DEFAULT_COOKIE_NAME + "=")) {
                requestBuilder.header(HttpHeaders.COOKIE, OidcConfig.DEFAULT_COOKIE_NAME + "=" + signedJwt.tokenContent());
            } else {
                requestBuilder.header(HttpHeaders.COOKIE, cookie.substring(0, cookie.indexOf(";")));
            }
        }
