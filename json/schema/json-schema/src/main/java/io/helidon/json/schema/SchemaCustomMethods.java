...
class SchemaCustomMethods {
     ...
     * @return schema related to the provided class
     */
    @Prototype.FactoryMethod
    static Schema get(Class<?> clazz) {
        return Services.first(JsonSchemaProvider.class, Qualifier.createNamed(clazz))
                .map(JsonSchemaProvider::schema)
                .orElseThrow(() -> new JsonSchemaException("No Schema found for the type: " + clazz.getName()));
    }
